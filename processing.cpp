...
// --- Divisions Submenu ---
const MenuItem kMenuDivisions[] = {
    {"Div",     TYPE_PARAM,   PARAM_DIVISION, nullptr,          0},
    {"< Back",  TYPE_BACK,    0,              kMenuMain,      0},
};
const int kMenuDivisionsSize = sizeof(kMenuDivisions) / sizeof(kMenuDivisions[0]);
...
const MenuItem kMenuMain[] = {
    ...
    {"Post",    TYPE_PARAM,         PARAM_POST_GAIN,    nullptr,        0},
    {"BPM",     TYPE_PARAM_SUBMENU, PARAM_BPM,          kMenuDivisions, kMenuDivisionsSize},
...
void Processing::Controls(Hardware &hw)
{
    ...
    // --- Encoder Click/Hold Logic ---

    // 1. Encoder is PRESSED
    ...
    {
        enc_is_holding = false; // Prevent re-triggering
        
        const MenuItem& item = GetSelectedItem();
        if(item.type == TYPE_PARAM_SUBMENU && ui_state == STATE_MENU_NAV)
        {
            // Enter submenu on hold
    ...
    // 3. Encoder is RELEASED (a "click")
    if(hw.encoder.FallingEdge())
    {
        if(enc_is_holding) // Only true if hold time was NOT reached
        {
            ...
            if (ui_state == STATE_MENU_NAV)
            {
                ...
                {
                    case TYPE_PARAM:
                    case TYPE_PARAM_SUBMENU: // Enters edit mode on CLICK
                    ...
                    case TYPE_SUBMENU:
                        ...
                        break;
                    case TYPE_BACK:
                        current_menu = item.submenu; // Points to parent
            ...
            else // STATE_PARAM_EDIT
            {
                ui_state = STATE_MENU_NAV; // Exit edit mode
            }
        }
    }
    ...
    if(inc != 0)
    {
        if(ui_state == STATE_MENU_NAV)
        {
            ...
            if(selected_item_idx < 0)
                selected_item_idx = current_menu_size - 1;
            if(selected_item_idx >= current_menu_size)
                selected_item_idx = 0;
...
// ... (UpdateBufferLen, UpdateGrainParams, GetSample) ...
